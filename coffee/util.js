...
/**
 * Stringify - 
 * Turns JSON  into string or returns undefined
 * @param {Object} json - Object to be stringified.
 * @example Example use of parse
 * @returns {String}
 */
export const stringify = object => {
  try { return JSON.stringify(object) } catch (e) {}
}
...
export function capitolise(s){
  return s.charAt(0).toUpperCase() + s.slice(1)
}

export function removeProps(obj, props){
  return Object.keys(obj).filter(key => !props.includes(key)).reduce((acc, key) => {
    acc[key] = obj[key]
    return acc
  }, {})
}